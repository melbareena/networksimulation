 ...
 * nodes.
 * 
 * @see <a href="http://en.wikipedia.org/wiki/Backpressure_routing">Backpressure
 *      Routing</a>
 * @author Benjamin
 */
public class BackPressureSchedulingStrategy extends SchedulingStrategy {
		...
		while (sourceBuffers.trafficSize() > 0 || transmitBuffers.trafficSize() > 0) {
			...
			if (sourceBuffers.trafficSize() == 0) {
				...
				Program.loadingDialog
						.setProgress((int) (100 - (99 * transmitBuffers
								.trafficSize() / maxTrafficTransmit)),
								"Disposing of transmit traffic ("
										+ transmitBuffers.trafficSize()
				...
				Program.loadingDialog
						.setProgress(
								...
								"Disposing of source traffic ("
										+ sourceBuffers.trafficSize()
										+ " remaining, timeslot " + timeSlot
										+ ")");
			}
		}
	...
	public SchedulingResult dynamicScheduling(long durationOfTrafficGenerating) {
		...
		sourceBuffers = null;
		do {
			totalTrafficGenerated = updateTraffic(0); // Fill the source buffers
														// with random traffic
		...
		while (sourceBuffers.trafficSize() > 0
			...
			for (Link link : tcu.getLinks()) {
				...
				if (sourceBuffers.containsKey(link)) {
					int dataRate = tcu.getRate(link);
					Packet moved = sourceBuffers.sendPacket(link, dataRate,
							transmitBuffers, timeSlot);
					if (moved.isReceived()) {
						double movedTraffic = moved.getTraffic();
						packetsDelay.add(moved.getDelay());
						slotThroughtput += movedTraffic;
				...
				if (transmitBuffers.containsKey(link)) {
					int dataRate = tcu.getRate(link);
					Packet moved = transmitBuffers.sendPacket(link, dataRate,
							transmitBuffers, timeSlot);
					if (moved.isReceived()) {
						double movedTraffic = moved.getTraffic();
						packetsDelay.add(moved.getDelay());
			...
			if (timeSlot < durationOfTrafficGenerating) {
				...
				Program.loadingDialog.setProgress(
						(int) (99 * timeSlot / durationOfTrafficGenerating),
						"Generating traffic (slot " + timeSlot + " over "
								+ durationOfTrafficGenerating + ")");
			} else {
				if (maxTrafficSource < 0) {
					maxTrafficSource = sourceBuffers.trafficSize();
					Program.loadingDialog.setProgress(0);
				...
				if (sourceBuffers.trafficSize() == 0) {
					...
					Program.loadingDialog
							.setProgress((int) (100 - (99 * sourceBuffers
									.trafficSize() / maxTrafficSource)),
									"Disposing of source traffic ("
											+ sourceBuffers.trafficSize()
											+ " remaining, timeslot "
											+ timeSlot + ")");
	 ...
	 * Note: A "commodity c data" is considered to be the data destined to node
	 * c.
	 * 
	...
	private Map<Link, Double> getOptimalWeightMap() {
		...
		for (Vertex a : nodeBuffersMap.keySet()) {
			...
			Map<Vertex, List<Packet>> aPacketDestMap = aBuffer
					.getPacketDestinationMap();
			// Looking at each a's neighbors b
			List<Link> neighborsLinks = TrafficEstimatingFacade
					.getOptimalLinks(a, LinkType.Outgoing);
			for (Link l : neighborsLinks) {
				...
				for (Vertex destination : aPacketDestMap.keySet()) {
					double aCommodityAmount = aBuffer
							.getTrafficTowardDestination(destination);
					double bCommodityAmount = (bBuffer != null) ? bBuffer
							.getTrafficTowardDestination(destination) : 0.0;
					// Refreshing the weight of link (a,b)
					abWeight = Math.max(aCommodityAmount - bCommodityAmount,
							abWeight);
				}
	 ...
	 * 
	 * @param weightsMap
	 *            The weight map: <em>Key: link (a,b) -> Value: weight</em>.
		...
		try {
			...
			for (TCUnit tcu : this.configurations) {
				...
				for (Link l : weightsMap.keySet()) {
					/*
					 * TODO Only add optimal path links...
					 */
					if (this.sourceBuffers.containsKey(l)
							|| this.transmitBuffers.containsKey(l)) {
	...
	private Buffer getBufferFromLink(Link l) {
		return this.sourceBuffers.containsKey(l) ? this.sourceBuffers.get(l)
				: this.transmitBuffers.get(l);
	}
